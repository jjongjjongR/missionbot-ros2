...
지금 단계에서 필요하지 않은 MoveIt2, rosbag2, LLM/VLM 개념을 미리 길게 설명하지 마라.
...
## 9. Phase 2 / Phase 3에서 반복된 gzclient crash 처리 기준
...
Phase 3와 Phase 4에서는 Gazebo GUI보다 RViz2 중심으로 진행했다.
...
예를 들어 Navigation2가 나오면 다음 수준으로 설명한다.

* 쉬운 설명: 저장된 지도 위에서 로봇이 목표 지점까지 이동하게 하는 시스템이다.
* 코드/명령어 관점: map 파일을 불러오고 Nav2 launch를 실행하며 RViz2에서 2D Pose Estimate와 2D Nav Goal을 사용한다.
* ROS2 구조 관점: map, amcl, planner, controller, costmap, behavior tree, lifecycle node들이 함께 동작한다.
* MissionBot 적용: 이후 모바일 매니퓰레이션에서 로봇이 작업 위치까지 이동하는 기반이 된다.
...
예를 들어 Phase 5에서 Navigation2 시작 전 환경을 확인하고 있다면,
갑자기 rosbag2, MoveIt2, LLM/VLM까지 설명하지 마라.

다만 한 문장 정도로 “이 개념은 나중에 어디에 쓰인다”는 연결은 해도 된다.

좋은 예시:

지금은 Navigation2로 저장된 map 위에서 목표 지점 이동을 확인하지만, MissionBot에서는 나중에 모바일 매니퓰레이터가 작업 위치까지 이동하는 기반이 된다.
...
다음 목표는 MissionBot-ROS2 Phase 5를 시작하는 것이다.
...
```text
Navigation2 시작 전 환경 및 map 파일 확인
```
...
먼저 Phase 5-1로, 현재 터미널에서 ROS2/Gazebo/RViz2/Navigation2/TurtleBot3 환경과 저장된 map 파일이 정상인지 확인하는 한 단계부터 안내해줘.
