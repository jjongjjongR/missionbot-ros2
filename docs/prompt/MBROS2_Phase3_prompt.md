...
지금 단계에서 필요하지 않은 Navigation2, MoveIt2, rosbag2, LLM/VLM 개념을 미리 길게 설명하지 마라.
...
이 흐름을 확인했고, 다음 Phase인 SLAM에서 /scan과 TF가 왜 중요한지 이해할 준비가 되었다.
```
...
## 8. Phase 2 / Phase 3에서 반복된 gzclient crash 처리 기준
...
다만 Phase 3에서는 Gazebo GUI보다 RViz2 중심으로 진행했다.
...
Gazebo가 실제 시뮬레이션을 실행한다면, RViz2는 topic, TF, 로봇 모델, 센서 데이터를 보여준다.
```
...
```text
Phase 4에서는 /scan과 TF를 이용해 SLAM Toolbox로 map을 생성한다.
Phase 3에서 확인한 /scan, /tf, /tf_static이 Phase 4의 기초가 된다.
```
...
예를 들어 SLAM이 나오면 다음 수준으로 설명한다.

* 쉬운 설명: 로봇이 움직이면서 주변 지도를 만드는 것이다.
* 코드/명령어 관점: slam_toolbox를 실행하고 /scan, /tf, /odom, /map을 확인한다.
* ROS2 구조 관점: LaserScan과 TF를 기반으로 map frame을 생성하고, map → odom 관계를 만든다.
* MissionBot 적용: 이후 Navigation2가 목표 지점까지 이동하기 위해 사용할 지도를 만드는 기초가 된다.
...
예를 들어 Phase 4에서 SLAM Toolbox를 확인하고 있다면,
갑자기 Navigation2, MoveIt2, rosbag2, LLM/VLM까지 설명하지 마라.

다만 한 문장 정도로 “이 개념은 나중에 어디에 쓰인다”는 연결은 해도 된다.

좋은 예시:

지금은 SLAM으로 map을 만들지만, MissionBot에서는 나중에 Navigation2가 이 map을 사용해 목표 지점까지 이동한다.
...
다음 목표는 MissionBot-ROS2 Phase 4를 시작하는 것이다.
...
먼저 Phase 4-1로, 현재 터미널에서 ROS2/Gazebo/RViz2/SLAM Toolbox/TurtleBot3 환경이 정상인지 확인하는 한 단계부터 안내해줘.
